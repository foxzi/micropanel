...
### Fixed
- API canAccessSite: check user role from DB instead of hardcoded user_id=1 for admin access
...
### Fixed
- Replace all remaining `nginx -s reload` references with `systemctl restart nginx` in default config, config files, docs, and scripts
- API CreateSite now applies `fix_mime_types` field from request body

## [1.3.9] - 2026-03-16

### Fixed
- Switch nginx reload from `nginx -s reload` to `systemctl restart nginx` to reliably pick up changes in included config files

## [1.3.8] - 2026-03-16

### Fixed
- hack.conf: files with %3F in URL but without ? in filename now served correctly via @strip_query fallback
- Added image file types (jpg, png, gif, webp, ico) to hack.conf MIME fix rules
...
### Changed
- WWW alias now disabled by default when creating sites
- Removed NoNewPrivileges from systemd service (required for sudo)

### Fixed
- SSL certificate status now readable via sudo cat
- SSL status auto-updates when viewing site page

## [1.2.12] - 2026-02-08

### Fixed
- Nginx config write/remove now uses sudo (fixes permission denied error)
- Added sudo permissions for tee and rm in /etc/nginx/sites-enabled/
...
### Fixed
- SSL certificate generation now uses --nginx method instead of --webroot
