...
func (r *SiteRepository) GetByID(id int64) (*models.Site, error) {
	site := &models.Site{}
	err := r.db.QueryRow(`
		SELECT id, name, owner_id, is_enabled, ssl_enabled, ssl_expires_at, ssl_cert_name, www_alias, created_at, updated_at
		FROM sites WHERE id = ?
	`, id).Scan(&site.ID, &site.Name, &site.OwnerID, &site.IsEnabled, &site.SSLEnabled, &site.SSLExpiresAt, &site.SSLCertName, &site.WWWAlias, &site.CreatedAt, &site.UpdatedAt)
...
func (r *SiteRepository) GetByName(name string) (*models.Site, error) {
	...
	`, name).Scan(&site.ID, &site.Name, &site.OwnerID, &site.IsEnabled, &site.SSLEnabled, &site.SSLExpiresAt, &site.SSLCertName, &site.WWWAlias, &site.CreatedAt, &site.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrNotFound
	}
	return site, err
...
func (r *SiteRepository) Create(site *models.Site) error {
	...
	result, err := r.db.Exec(`
		INSERT INTO sites (name, owner_id, is_enabled, ssl_enabled, ssl_expires_at, ssl_cert_name, www_alias, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, site.Name, site.OwnerID, site.IsEnabled, site.SSLEnabled, site.SSLExpiresAt, site.SSLCertName, site.WWWAlias, now, now)
	if err != nil {
		return err
	}
...
func (r *SiteRepository) Update(site *models.Site) error {
	...
	_, err := r.db.Exec(`
		UPDATE sites SET name = ?, is_enabled = ?, ssl_enabled = ?, ssl_expires_at = ?, ssl_cert_name = ?, www_alias = ?, updated_at = ?
		WHERE id = ?
	`, site.Name, site.IsEnabled, site.SSLEnabled, site.SSLExpiresAt, site.SSLCertName, site.WWWAlias, site.UpdatedAt, site.ID)
	return err
}
...
func (r *SiteRepository) scanSites(rows *sql.Rows) ([]*models.Site, error) {
	...
	for rows.Next() {
		site := &models.Site{}
		if err := rows.Scan(&site.ID, &site.Name, &site.OwnerID, &site.IsEnabled, &site.SSLEnabled, &site.SSLExpiresAt, &site.SSLCertName, &site.WWWAlias, &site.CreatedAt, &site.UpdatedAt); err != nil {
			return nil, err
		}
